...
public class BankingProductEligibility
    extends io.biza.cdr.babelfish.model.banking.BankingProductEligibility {
  ...
  private boolean isInfoDefined() {
    return Arrays.asList(new BankingProductEligibilityType[] {
        BankingProductEligibilityType.OTHER
  ...
  private boolean isValuePositiveInteger() {
    return Arrays.asList(new BankingProductEligibilityType[] {
        BankingProductEligibilityType.MIN_AGE, BankingProductEligibilityType.MAX_AGE
    }).contains(eligibilityType()) ? FormatChecker.isPositiveInteger(additionalValue()) : true;
  }
