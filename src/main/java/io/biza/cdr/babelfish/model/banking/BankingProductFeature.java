...
public class BankingProductFeature {
    ...
    @BabelFishModelProperty(description = "Link to a web page with more information on this feature", dataType = "java.lang.String")
    ...
    private boolean isInfoDefined() {
    	return Arrays.asList(new BankingProductFeatureType[] { BankingProductFeatureType.OTHER}).contains(featureType) 
    ...
    private boolean isValueDuration() {
    	return Arrays.asList(new BankingProductFeatureType[] { BankingProductFeatureType.INTEREST_FREE, BankingProductFeatureType.INTEREST_FREE_TRANSFERS}).contains(featureType)
                ? FormatChecker.isDuration(additionalValue)
                : true;
    }

    @AssertTrue(message = "Additional Value must be a Positive Integer when Feature type is FREE_TXNS or BONUS_REWARDS")
    private boolean isValuePositiveInteger() {
        return Arrays.asList(new BankingProductFeatureType[] { BankingProductFeatureType.FREE_TXNS, BankingProductFeatureType.BONUS_REWARDS}).contains(featureType)
                ? FormatChecker.isPositiveInteger(additionalValue)
                : true;
    }

    @AssertTrue(message = "Additional Value must be an Amount String when Eligibility type is FREE_TXNS_ALLOWANCE")
    private boolean isValueAmount() { 
    	return Arrays.asList(new BankingProductFeatureType[] { BankingProductFeatureType.FREE_TXNS_ALLOWANCE}).contains(featureType)
